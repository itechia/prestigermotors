        ...
        {isLoading ? (
          ...
          <div className="divide-y divide-border/50">
            {filtered.map((v) => {
              ...
              return isAdmin ? (
                <Link key={v.id} href={`/admin/veiculo/${v.id}`} className="flex items-center gap-4 p-4 hover:bg-secondary/50 transition-colors">
                  {content}
                </Link>
              ) : (
                <div key={v.id} className="flex items-center gap-4 p-4">
                  ...
                  <Button asChild size="sm" variant="outline" className="rounded-full shrink-0">
                    <Link href={`/admin/vendas?veiculo=${v.id}`}>
                      <ReceiptText className="w-4 h-4 mr-2" /> Venda
                    </Link>
                  </Button>
                </div>
              );
